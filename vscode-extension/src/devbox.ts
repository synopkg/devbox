...
export async function devboxReopen() {
    ...
    if (process.platform === 'win32') {
        ...
        const result = await window.showErrorMessage(
            ...
            Please open VSCode from inside synopkg.github.io/devboxell in WSL using the CLI.', seeDocs
        );
        if (result === seeDocs) {
            env.openExternal(Uri.parse('https://synopkg.github.io/devbox/docs/ide_configuration/vscode/#windows-setup'));
            return;
        }
    }
...
async function setupDotDevbox(workingDir: Uri, dotdevbox: Uri) {
    try {
        ...
        // running synopkg.github.io/devboxellenv to create it
