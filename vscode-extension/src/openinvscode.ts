...
async function getVMInfo(token: string | null, vmId: string | null): Promise<any> {
    ...
    const gatewayHost = 'https://api.synopkg.github.io/devbox/g/vm_info';
    ...
    console.debug("API Call to api.synopkg.github.io/devbox response: " + response);
    return response;
}

async function setupDevboxLauncher(): Promise<any> {
    // download devbox launcher script
    const gatewayHost = 'https://raw.githubusercontent.com/synopkg/devbox/main/devbox';
    const response = await fetch(gatewayHost, {
        method: 'get',
    });
