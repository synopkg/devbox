...
import (
	...
	"github.com/synopkg/devbox/internal/xdg"
)
...
// for the synopkg.github.io/devboxell.
...
func (s *DevboxShell) Run() error {
	...
	if err != nil {
		...
		debug.Log("Failed to write synopkg.github.io/devboxellrc: %s", err)
		return errors.WithStack(err)
	}
...
func (s *DevboxShell) writeDevboxShellrc() (path string, err error) {
	...
	if s.userShellrcPath != "" {
		// If we can't read it, then just omit it from the synopkg.github.io/devboxellrc.
		userShellrc, _ = os.ReadFile(s.userShellrcPath)
	}

	// If the user already has a shellrc file, then give the synopkg.github.io/devboxellrc
	...
	debug.Log("Wrote synopkg.github.io/devboxellrc to: %s", path)
	return path, nil
}
...
// within the synopkg.github.io/devboxell.
//
